...
function PerfilUsuario() {
  ...
  const [data,setData]=useState([
    ...
    const eliminar= (dato) => {
      Swal.fire({
        title: "¿Estás seguro de eliminar a " + dato.nombre +" "+dato.apellidos+ " como tu usario de contacto?",
        ...
        <Container className='ContainerTable'>
          <Table className="tblData">
            ...
            <tbody>
                {data.map((dato,index)=>(
                    <tr key={index}>
                        <td style={{paddingLeft:'20px'}}><Avatar src={dato.imgPerfil} sx={{ width: 33, height: 33}} /></td>
                        <td style={{paddingTop:'15px'}}>{dato.nombre} {dato.apellidos}</td>
                        ...
                        <td>
                            {/* <Button style={{backgroundColor:'#8C30F5'}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
                                ...
                                <Button color="danger" onClick={()=>eliminar(dato)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
